...
{   
    ...
    this.anims.create({
        key: 'playerKilled',
        frames: this.anims.generateFrameNumbers('playerKilled', { start: 0, end: 5 }),
        frameRate: 120,
        repeat: 0
    });
    ...
    this.physics.add.collider(player, movingPlatform, ()=>{player.body.blocked.down = true}, null, this);
...
{   
    ...
    {   
        ...
        if(player.body.blocked.down) {
            ...
            player.setVelocityY(-playerJumpVelocity);
            player.anims.play('jump', true);
            if (cursors.right.isDown){player.anims.play('jumpRight', true);}
            if (cursors.left.isDown){player.anims.play('jumpLeft', true);}
    ...
    if (this.input.pointer1.isDown || this.input.pointer2.isDown) {
        if (this.input.pointer1.position.x < game.config.width/4 || this.input.pointer2.position.x < game.config.width/4 && this.input.pointer2.position.x > 0) {
            if (playerActualVelocity < playerVelocity) {
                playerActualVelocity += 50;
            }
            player.setVelocityX(-playerActualVelocity);
        ...
        } else if(this.input.pointer1.position.x > game.config.width/4 && this.input.pointer1.position.x < game.config.width/2 ||
                    this.input.pointer2.position.x > game.config.width/4 && this.input.pointer2.position.x < game.config.width/2) {
            if (playerActualVelocity < playerVelocity) {
                playerActualVelocity += 50;
            }
            player.setVelocityX(playerActualVelocity);
        ...
        if (this.input.pointer1.justDown && this.input.pointer1.position.x > game.config.width/2 || this.input.pointer2.justDown && this.input.pointer2.position.x > game.config.width/2) {
            if(player.body.blocked.down) {
                canDoubleJump = true;
                player.setVelocityY(-playerJumpVelocity);
            } else if (!player.body.blocked.down && canDoubleJump) {
